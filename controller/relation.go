...
func RelationAction(c *gin.Context) {
	...
	if actionType == "1" {
		...
		//把对方用户从关注列表里删除
		//把当前用户从对方用户的粉丝列表里删除
		dao.DB.Where("follow_id = ? and follower_id = ?", user.Id, toUserId).Delete(Relation{})
...
func FollowList(c *gin.Context) {
	...
	//找出当前用户的结构体
	dao.DB.Where("follow_id = ?", userId).Find(&relationsFollow)
	//把当前用户关注的对方用户放到切片relations里
	//在关系表里查询
	//每一次都要重新清零，不然会一直在切片末尾追加，无法改变前面已添加的
	toUserIdsFollow = []int64{}
	for i := 0; i < len(relationsFollow); i++ {
		toUserIdsFollow = append(toUserIdsFollow, relationsFollow[i].FollowerId)
	}
	//从User表里用对方的ID找出对应的结构体
	//在用户表里查询，不是关系表
	//根据切片toUserIds里的对方用户ID直接查询，放在UserList切片
	dao.DB.Where(toUserIdsFollow).Find(&UserListFollow) // gorm写法
	fmt.Println(UserListFollow)
	c.JSON(http.StatusOK, UserListResponse{
		Response: Response{
			StatusCode: 0,
		},
		UserList: UserListFollow,
	})
}
...
func FollowerList(c *gin.Context) {
	...
	dao.DB.Where("follower_id = ?", userId).Find(&relationsFollower)
	//把当前用户关注的followid放到切片relations里
	//每一次都要重新清零，不然会一直在切片末尾追加，无法改变前面已添加的
	toUserIdsFollower = []int64{}
	for i := 0; i < len(relationsFollower); i++ {
		//粉丝列表是找FollowId，和关注列表相反
		//这里的toUserIds还是对方用户的Id，只不过现在对方用户Id在粉丝列表中
		toUserIdsFollower = append(toUserIdsFollower, relationsFollower[i].FollowId)
	}
	//从User表里用对方的ID找出对应的结构体
	//在用户表里查询，不是关系表
	//根据切片toUserIds里的对方用户ID直接查询，放在UserList切片
	dao.DB.Where(toUserIdsFollower).Find(&UserListFollower) // gorm写法
	c.JSON(http.StatusOK, UserListResponse{
		Response: Response{
			StatusCode: 0,
		},
		UserList: UserListFollower,
	})
}
